...
other_bank_appfs_encryption()
{
	...
	if [[ $other_bank_device != "" ]]; then
		...
		# As the mount point is not mounted as ecryptfs yet, being able to find the pattern "nebulaURL"
		# in the file nebula.properties_hachtest is an indication that the system is not encrypted
		grep -c "nebulaURL" "$other_bank_mount_point/set/nebula.properties_hachtest" > /dev/null && encrypted=false || encrypted=true

		# Handle the case in which the reserved directory used for encryption
		# already exists. It would be renamed with a free name generated by mktemp, to backup data
		backuped_unencrypted_data=false
		if [ $encrypted == false ] && [ -e "$other_bank_mount_point/$ENCRYPTED_DIRECTORY" ]; then
			# The flag "-u" means "dry-run"; only the name is generated
			new_name=`mktemp -u "$other_bank_mount_point/$ENCRYPTED_DIRECTORY.XXXXXX"`
			mv "$other_bank_mount_point/$ENCRYPTED_DIRECTORY" "$new_name"
			backuped_unencrypted_data=true
		fi

		# On the first time the encryption is enabled on a partition, it is set up so that specified
		# files or directories become encrypted. Hence the call to enable_encryption_on() directly
		# followed by umounts
		enable_encryption_on "$other_bank_mount_point"

		# Restore the unencrypted backuped data into the final encrypted directory, and remove the backuped one
		# || : is added to let script continue in case execution of the commad causes any error
		if [ $backuped_unencrypted_data == true ]; then
			cp -rpf $new_name/. $other_bank_mount_point/$ENCRYPTED_DIRECTORY || :
			rm -rf $new_name || :
		fi

		umount "$other_bank_mount_point/$ENCRYPTED_DIRECTORY"
